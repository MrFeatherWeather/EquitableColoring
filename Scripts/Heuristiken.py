...
def FJK(G:nx.Graph,coloring): #Bemerkung: dies ist tatsächlich nicht der FJK-Algoritmus sondern der Algorithmus "Naive" aus dem gleichen Paper
...
def FJK2(G:nx.Graph,coloring): #Bemerkung: dies ist der FJK-Algoritmus
    MAX_COLOR = max(coloring.values())
    color_freq = [0]*(MAX_COLOR+1)
    color_groups = {k: [] for k in range(MAX_COLOR + 1)}
    for v,c in coloring.items():
...
def FJK3(G:nx.Graph,coloring): #Bemerkung: dies ist der FJK2-Algoritmus
    MAX_COLOR = max(coloring.values())
    color_freq = [0]*(MAX_COLOR+1)
    color_groups = {k: [] for k in range(MAX_COLOR + 1)}
    for v,c in coloring.items():
