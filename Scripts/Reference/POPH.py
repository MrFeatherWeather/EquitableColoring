...
def color(G, PRINT=0, TLIM=600, solution=None):  # {
    ...
    if PRINT & 4: print
    'Reduction (V,E): (%s,%s) --> (%s,%s)    %0.2F sec  ' % (
        len(V_0), len(E_0), len(V), len(E), (time.perf_counter()- c00));
    ...
    print(Qq)
    if PRINT & 4:    print
    'Time Qq:', (time.perf_counter() - c0)
    c0 = time.perf_counter()

    lb = len(Qq)

    if PRINT & 4:    print
    '(V,E,lb,ub,H) = (%s,%s,%s,%s,%s)' % (len(V), len(E), lb, ub, H)
    ...
    if PRINT & 4:    print
    'Time write IP:', (time.perf_counter() - c0)
...
def best_clique(G, H, PRINT):  # {
    ...
    for i in range(int(max_try)):
        ...
        if bestQplusCutQ < QplusCutQ:
            ...
            string = '\t|Q|: %s   #Vars: %s/%s     %s sec\r' % (
            lenQ, remaining_number_vars, number_vars, time.perf_counter() - t0)
            if PRINT & 8:
...
color(prs.readDimacs("../../Instances/Error/3-FullIns_4.col"),PRINT=0x001)